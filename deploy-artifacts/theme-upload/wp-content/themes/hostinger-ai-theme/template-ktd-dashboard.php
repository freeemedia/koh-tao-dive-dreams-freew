...
<head>
  ...
  <!-- Google tag (gtag.js) -->
  <script async src="https://www.googletagmanager.com/gtag/js?id=G-PC1X72DKC5"></script>
  <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());

    gtag('config', 'G-PC1X72DKC5');
  </script>
  <?php wp_head(); ?>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge-deposit_paid{ background: rgba(245,158,11,.18); color: var(--yellow); }
    ...
    .delete-btn { background: rgba(239,68,68,.15); border:1px solid rgba(239,68,68,.35); color: var(--red); border-radius:5px; padding:3px 8px; font-size:11px; cursor:pointer; transition:opacity .15s, border-color .15s; white-space:nowrap; }
    .delete-btn:hover { border-color: var(--red); opacity: .9; }
...
<div id="ktd-shell">
  ...
  <aside id="ktd-sidebar">
    ...
    <ul class="sidebar-nav">
      <li><a href="https://divinginasia.com" target="_blank">
        <span class="nav-icon">🌐</span> Live Site ↗
      </a></li>
      <li><a href="https://www.trip.com/t/o7EkVJcD3V2" target="_blank" rel="noopener noreferrer">
        <span class="nav-icon">📅</span> Trip.com Affiliate ↗
      </a></li>
    </ul>
          ...
          <div class="panel" id="bookings-section">
            <div class="panel-header">
              ...
              <div class="header-actions">
                ...
                <a href="https://dashboard.stripe.com" target="_blank" class="btn btn-ghost" style="padding:5px 12px;font-size:12px;">Stripe</a>
              </div>
            </div>
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
          ...
          <div class="panel">
            ...
            <div class="quick-actions">
              <a href="https://www.trip.com/t/o7EkVJcD3V2" target="_blank" rel="noopener noreferrer" class="action-item">
                <span class="action-icon">📅</span>
                <span class="action-label">Open Trip.com Affiliate</span>
              ...
              <a href="https://divinginasia.com" target="_blank" class="action-item">
                <span class="action-icon">🌐</span>
...
(function () {
  ...
  async function deleteBooking(id) {
    const res = await fetch(REST_BASE + '/bookings/' + id, {
      method: 'DELETE',
      headers: { 'X-WP-Nonce': NONCE, ...(API_KEY ? { 'x-ktd-api-key': API_KEY } : {}) },
    });
    if (!res.ok) { const e = await res.json().catch(()=>({})); throw new Error(e.message || 'HTTP ' + res.status); }
    return res.json();
  }
  ...
  const wpInvoiceBaseUrl = '<?php echo esc_js( admin_url( 'admin.php?page=ktd-invoice&id=' ) ); ?>';

  function normalizeBookingStatus(status) {
    const s = (status || 'new').toString().trim().toLowerCase();
  ...
  function statusBadge(status) {
    ...
    const map = { new: 'badge-new', confirmed: 'badge-confirmed', deposit_paid: 'badge-deposit_paid', cancelled: 'badge-cancelled', completed: 'badge-completed' };
    return `<span class="badge ${map[s] || 'badge-new'}">${bookingStatusLabel(s)}</span>`;
  }
  ...
  function normalizeNotes(val) {
    const raw = String(val || '').trim();
    const key = raw.toLowerCase().replace(/[^a-z]/g, '');
    if (!raw || key === 'wordpress' || key === 'wordrpress' || key.startsWith('wordpress')) return '';
    return raw;
  }
  ...
  function renderTable(bookings) {
      ...
      <tr data-id="${b.id}">
        ...
        <td>
          <select class="ktd-status-select" data-id="${b.id}"
            ...
            ${['new','confirmed','deposit_paid','completed','cancelled'].map(s =>
              `<option value="${s}"${normalizeBookingStatus(b.status)===s?' selected':''}>${bookingStatusLabel(s)}</option>`
            ).join('')}
          </select>
        </td>
        <td>
        ...
        <td>
          <button class="invoice-btn ktd-invoice-btn" data-id="${b.id}" data-name="${escHtml(b.name||'')}" data-email="${escHtml(b.email||'')}"
            data-total="${b.total_amount||''}" data-deposit="${b.deposit_amount||''}" data-title="${escHtml(b.item_title||b.booking_type||'')}"
            title="Open WordPress invoice">🧾 Invoice</button>
          <button class="invoice-btn paypal-inv-btn" data-id="${b.id}" data-name="${escHtml(b.name||'')}" data-email="${escHtml(b.email||'')}"
            ...
            title="Send PayPal payment link">🅿 PayPal</button>
          <button class="delete-btn ktd-delete-btn" data-id="${b.id}" title="Delete booking permanently" style="display:block;margin-top:4px;">Delete</button>
        </td>
        <td style="min-width:160px;">
          <textarea class="ktd-notes-input" data-id="${b.id}" rows="2"
            ...
            placeholder="Add notes…">${escHtml(normalizeNotes(b.internal_notes || b.comments || b.message || ''))}</textarea>
        </td>
      </tr>
    `).join('');
    ...
    tbody.querySelectorAll('.ktd-notes-input').forEach(ta => {
      ta.addEventListener('blur', async function() {
        const id    = this.dataset.id;
        const notes = normalizeNotes(this.value);
        this.value = notes;
        const bk    = allBookings.find(b => String(b.id) === String(id));
        if (bk && normalizeNotes(bk.internal_notes || bk.comments || bk.message || '') === notes) return;
        try {
          await patchBooking(id, { internal_notes: notes, comments: notes, message: notes });
          if (bk) {
            bk.internal_notes = notes;
            bk.comments = notes;
            bk.message = notes;
          }
        } catch(e) {
    ...
    // Delete booking
    tbody.querySelectorAll('.ktd-delete-btn').forEach(btn => {
  ...
  function openPrintableInvoice(booking) {
    const id = booking.id || '—';
    const customerName = booking.name || '—';
    const customerEmail = booking.email || '';
    const customerPhone = booking.phone || '';
    const title = booking.course_title || booking.item_title || booking.booking_type || 'Service';
    const date = booking.preferred_date || (booking.created_at ? String(booking.created_at).slice(0, 10) : '—');
    const status = (booking.status || 'new').toLowerCase();
    const paymentStatus = (booking.payment_status || 'unpaid').toLowerCase();
    const total = parseFloat(booking.total_amount || 0) || 0;
    const deposit = parseFloat(booking.deposit_amount || 0) || 0;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Invoice ${escHtml(invNum)}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .inv-header .company p { color: #555; margin-top: 4px; font-size: 12px; line-height: 1.5; }
    ...
    .badge-pending, .badge-new { background: #fff3cd; color: #856404; }
    .badge-cancelled { background: #f8d7da; color: #721c24; }
    .badge-completed { background: #d1ecf1; color: #0c5460; }
    .badge-paid { background: #d4edda; color: #155724; }
    .badge-invoiced { background: #fff3cd; color: #856404; }
...
<body>
  <div class="invoice-wrap">
    <div class="inv-header">
      <div class="company">
        ...
        <p>Koh Tao, Surat Thani, Thailand<br>Email: info@divinginasia.com<br>Web: www.divinginasia.com</p>
      </div>
      <div class="inv-meta">
        <h2>INVOICE</h2>
        <p><strong>${escHtml(invNum)}</strong></p>
        <p>Issued: ${escHtml(today)}</p>
        <p>
          <span class="badge badge-${escHtml(status)}">${escHtml(status)}</span>
          &nbsp;
          <span class="badge badge-${escHtml(paymentStatus)}">${escHtml(paymentStatus)}</span>
        </p>
      </div>
    </div>
    ...
    <div class="inv-parties">
      <div class="party">
        <h3>Bill To</h3>
        <p><strong>${escHtml(customerName)}</strong><br>${escHtml(customerEmail)}<br>${escHtml(customerPhone)}</p>
      </div>
      <div class="party">
        <h3>Service Date</h3>
        <p>${escHtml(date)}</p>
      ...
      <div class="party">
        <h3>Booking Type</h3>
        <p>${escHtml(booking.booking_type || '—')}</p>
      </div>
    </div>

    <table>
      <thead><tr><th>Description</th><th width="80">Qty</th><th width="140">Amount (THB)</th></tr></thead>
      <tbody><tr><td>${escHtml(title)}</td><td>1</td><td>${escHtml(total.toFixed(2))}</td></tr></tbody>
    </table>

    <div class="inv-totals">
      <table>
        <tr><td>Subtotal</td><td align="right">${escHtml(total.toFixed(2))} THB</td></tr>
        <tr><td>Deposit Paid</td><td align="right">- ${escHtml(deposit.toFixed(2))} THB</td></tr>
        <tr class="grand-total"><td>Balance Due</td><td align="right">${escHtml(due.toFixed(2))} THB</td></tr>
      </table>
    </div>

    <div class="inv-footer">Thank you for diving with Pro Diving Asia! | www.divinginasia.com</div>
  ...
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('.ktd-invoice-btn');
    if (!btn) return;
    const invoiceUrl = wpInvoiceBaseUrl + encodeURIComponent(btn.dataset.id || '');
  ...
  function applyFilters() {
    ...
    const filtered = allBookings.filter(b => {
      ...
      if (status && normalizeBookingStatus(b.status) !== status) return false;
