...
<div id="ktd-shell">
  ...
  <aside id="ktd-sidebar">
    ...
    <ul class="sidebar-nav">
      ...
      <li><a href="https://divinginasia.com/booking" target="_blank">
        ...
        <div style="display:flex;flex-direction:column;gap:20px;">
          ...
          <div class="panel">
            ...
            <div class="quick-actions">
              <a href="https://divinginasia.com/booking" target="_blank" class="action-item">
...
(function () {
  ...
  function renderTable(bookings) {
    ...
    tbody.innerHTML = bookings.slice(0, 50).map(b => `
      <tr data-id="${b.id}">
        ...
        <td>
          <button class="delete-btn ktd-delete-btn" data-id="${b.id}" title="Delete booking permanently">Delete</button>
        </td>
      </tr>
    `).join('');
  ...
  async function handleMollieInvoiceSend(buttonEl) {
    ...
    try {
      const resp = await fetch('https://divinginasia.com/api/mollie-payment', {
        method: 'POST',
        headers: {
  ...
  function renderActivity(bookings) {
    ...
    const dotColor = { new: '#6366f1', confirmed: '#10b981', deposit_paid: '#f59e0b', cancelled: '#ef4444', completed: '#3b82f6' };

    list.style.display = 'block';
    list.innerHTML = recent.map(b => {
      const status = normalizeBookingStatus(b.status);
      const color = dotColor[status] || '#6366f1';
      return `
        <li>
          <div class="activity-dot" style="background:${color};"></div>
          <div class="activity-content">
            <div class="activity-name">${escHtml(b.name || 'Unknown')}</div>
            <div class="activity-detail">${escHtml(b.item_title || b.booking_type || 'Booking')} · ${escHtml(bookingStatusLabel(status))}</div>
          </div>
